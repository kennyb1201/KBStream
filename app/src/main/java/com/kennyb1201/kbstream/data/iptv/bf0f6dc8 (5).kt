...
class IptvRepository(
    ...
    suspend fun loadLineup(
        ...
        val guide = epgUrl?.let { url ->
            runCatching {
                ...
                Log.d(
                    TAG,
                    "EPG loaded: url=$url channels=${parsedGuide.channels.size} programs=${parsedGuide.programs.size}"
                )
            }.onFailure { throwable ->
        ...
        if (guide == null) {
            Log.w(
                TAG,
                "Guide is null after loadLineup. playlistChannels=${playlist.channels.size} epgUrl=${epgUrl.orEmpty()}"
            )
        } else {
            ...
            mapped.asSequence()
                .filter { it.epgMatchType == EpgMatchType.NO_MATCH }
                .take(10)
                .forEach { item ->
                    Log.d(
                        TAG,
                        "Unmatched channel: name=${item.channel.displayName} tvgId=${item.channel.tvgId.orEmpty()} tvgName=${item.channel.tvgName.orEmpty()} providerId=${item.channel.providerChannelId.orEmpty()}"
                    )
                }

            mapped.asSequence()
                .filter { it.epgMatchType != EpgMatchType.NO_MATCH && it.now == null && it.next == null && it.upcoming.isEmpty() }
                .take(10)
                .forEach { item ->
                    Log.d(
                        TAG,
                        "Matched but no program data: name=${item.channel.displayName} epgId=${item.epgChannel?.id.orEmpty()} displayNames=${item.epgChannel?.displayNames.orEmpty()}"
                    )
                }
        }
    ...
    private suspend fun fetchAndParseGuide(url: String): XmltvGuide = withContext(Dispatchers.IO) {
        ...
        client.newCall(request).execute().use { response ->
            ...
            if (!response.isSuccessful) {
                val preview = response.peekBody(1024 * 4).string().take(300)
                error(
            ...
            val buffered = BufferedInputStream(responseBody.byteStream())
            buffered.mark(2)
            ...
            Log.d(
                TAG,
                "EPG fetch streaming: url=$url contentType=$contentType contentEncoding=$contentEncoding gzip=$isGzip"
            )
