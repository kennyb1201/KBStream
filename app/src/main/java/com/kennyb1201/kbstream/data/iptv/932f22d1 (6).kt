    ...
    fun parse(reader: Reader, sourceUrl: String? = null): XmltvGuide {
        ...
        while (eventType != XmlPullParser.END_DOCUMENT) {
            if (eventType == XmlPullParser.START_TAG) {
                when (parser.name) {
                    "channel" -> {
                        channels += readChannel(parser)
                        channelCount++
                        if (channelCount % 1000 == 0) {
                            Log.d(TAG, "XMLTV progress: parsedChannels=$channelCount parsedProgrammes=$programmeCount source=${sourceUrl.orEmpty()}")
                        }
                    }
                    "programme" -> {
                        programs += readProgram(parser)
                        programmeCount++
        ...
        Log.d(TAG, "XMLTV completed: parsedChannels=$channelCount parsedProgrammes=$programmeCount source=${sourceUrl.orEmpty()}")
