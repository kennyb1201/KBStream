...
class XmltvImporter(
    ...
    private fun readChannel(parser: XmlPullParser, sourceUrl: String): EpgChannelEntity? {
        ...
        if (channelDetailLogsRemaining > 0) {
            Log.e(TAG, "READ CHANNEL BUILD id=$id names=${cleanNames.size} aliases=${aliasKeys.size} icon=${iconUrl != null}")
        ...
        while (true) {
            when (parser.next()) {
                ...
                XmlPullParser.END_DOCUMENT -> {
                    Log.e(TAG, "READ PROGRAM HIT END_DOCUMENT index=$programIndex steps=$steps")
                    return null
                }
