...
class XmltvImporter(
    ...
    suspend fun import(
        ...
        if (!IMPORT_IN_PROGRESS.compareAndSet(false, true)) {
            Log.w(TAG, "IMPORT SKIPPED duplicate import source=$sourceUrl")
            return
        }
        ...
        try {
            ...
            while (eventType != XmlPullParser.END_DOCUMENT) {
                if (eventType == XmlPullParser.START_TAG) {
                    ...
                    if ((parsedChannels < 3 && parsedPrograms == 0) || parsedPrograms < 3) {
                    ...
                    when (tagName) {
                        "channel" -> {
                            ...
                            if (channelBatch.size >= CHANNEL_BATCH_SIZE) {
                                Log.e(TAG, "INSERT CHANNEL BATCH START size=${channelBatch.size}")
                                dao.insertChannels(channelBatch.toList())
                                channelBatch.clear()
                                Log.e(TAG, "INSERT CHANNEL BATCH END")
                            }
