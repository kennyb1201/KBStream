...
private fun String.parsedLocalDate(): java.time.LocalDate? =
    takeIf { it.isNotBlank() }?.let { runCatching { java.time.LocalDate.parse(it) }.getOrNull() }

fun TmdbPersonDetail.formattedBirthday(): String? =
    birthday?.parsedLocalDate()?.format(java.time.format.DateTimeFormatter.ofPattern("MMM d, yyyy"))
        ?: birthday?.takeIf { it.isNotBlank() }

fun TmdbPersonDetail.formattedDeathday(): String? =
    deathday?.parsedLocalDate()?.format(java.time.format.DateTimeFormatter.ofPattern("MMM d, yyyy"))
        ?: deathday?.takeIf { it.isNotBlank() }

/**
 * Age in years -- as of death if deathday is set, otherwise as of today.
 * Null if birthday is missing or unparsable.
 */
...
/**
 * A single-line "known for • born ... (age ..) • place of birth" summary,
 * skipping any piece that TMDB didn't return. Empty string if nothing is available.
 */
