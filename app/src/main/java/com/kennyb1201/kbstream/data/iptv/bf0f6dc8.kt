...
class IptvRepository(
    ...
    fun mapChannels(
        ...
        val guideChannelsByName = guide.channels
            .flatMap { channel ->
                channel.displayNames.map { normalizeName(it) to channel }
            }
            .toMap()

        val programmesByChannel = guide.programmes.groupBy { it.channelId.trim() }

        return playlist.channels.map { channel ->
            val epgChannel = resolveGuideChannel(channel, guideChannelsById, guideChannelsByName)
            val programmes = epgChannel?.let { programmesByChannel[it.id.trim()].orEmpty() }.orEmpty()
