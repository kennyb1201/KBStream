    ...
    private suspend fun loadProgramsChunked(
        ...
        for (batch in channelIds.chunked(PROGRAM_QUERY_BATCH_SIZE)) {
            ...
            val batchRows = withContext(Dispatchers.IO) {
                ...
                .take(remainingLimit.coerceAtMost(MAX_PROGRAM_ROWS_PER_BATCH))

            Log.w(
                TAG,
                "PROGRAM BATCH channels=${batch.size} rows=${batchRows.size} remaining=$remainingLimit"
            )
...
", "", "|")
    ...
    private companion object {
        ...
        const val MAX_PROGRAM_ROWS_PER_BATCH = 24
