...
class TmdbRepository {
    ...
    suspend fun getByGenre(genreId: Int): List<StudioSection> {
        ...
        return listOf(
            StudioSection("MOVIES · RECENT", discoverByGenre("movie", genreId, "primary_release_date.desc")),
            StudioSection("MOVIES · POPULAR", discoverByGenre("movie", genreId, "popularity.desc")),
            StudioSection("MOVIES · TOP RATED", discoverByGenre("movie", genreId, "vote_average.desc", voteCountGte = 100)),
            StudioSection("SERIES · RECENT", discoverByGenre("series", genreId, "first_air_date.desc")),
            StudioSection("SERIES · POPULAR", discoverByGenre("series", genreId, "popularity.desc")),
            StudioSection("SERIES · TOP RATED", discoverByGenre("series", genreId, "vote_average.desc", voteCountGte = 100))
        ).filter { it.items.isNotEmpty() }
    }
    ...
    suspend fun getByKeyword(keywordId: Int): List<StudioSection> {
        ...
        suspend fun fetchMovies(sortBy: String): List<StudioItem> =
            (1..pagesPerRail).flatMap { page ->
                runCatching { api.discoverMovieByKeyword(keywordId, apiKey, sortBy, page).results }.getOrDefault(emptyList())
            }.distinctBy { it.id }.map { StudioItem(it, "movie") }

        suspend fun fetchShows(sortBy: String): List<StudioItem> =
            (1..pagesPerRail).flatMap { page ->
                runCatching { api.discoverTvByKeyword(keywordId, apiKey, sortBy, page).results }.getOrDefault(emptyList())
            }.distinctBy { it.id }.map { StudioItem(it, "series") }

        return listOf(
            StudioSection("MOVIES · RECENT", fetchMovies("primary_release_date.desc")),
            StudioSection("MOVIES · POPULAR", fetchMovies("popularity.desc")),
            StudioSection("MOVIES · TOP RATED", fetchMovies("vote_average.desc")),
            StudioSection("SERIES · RECENT", fetchShows("first_air_date.desc")),
            StudioSection("SERIES · POPULAR", fetchShows("popularity.desc")),
            StudioSection("SERIES · TOP RATED", fetchShows("vote_average.desc"))
        ).filter { it.items.isNotEmpty() }
    }
