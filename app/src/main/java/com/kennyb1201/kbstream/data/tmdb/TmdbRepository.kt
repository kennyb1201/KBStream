...
class TmdbRepository {
    ...
    // US certification (e.g. "PG-13" for movies, "TV-MA" for series). Falls back to
    // null if no US rating is present so the UI can hide the badge gracefully.
    fun getCertification(detail: TmdbDetail, region: String = "US"): String? {
        detail.contentRatings?.results?.firstOrNull { it.country == region }?.rating?.let { return it }
        return detail.releaseDates?.results
            ?.firstOrNull { it.country == region }
            ?.releaseDates
            ?.firstOrNull { it.certification.isNotBlank() }
            ?.certification
    }

    // Prefers an English transparent logo; falls back to the first logo available.
    fun getBestLogo(detail: TmdbDetail, language: String = "en"): String? {
        val logo = detail.images?.logos?.firstOrNull { it.language == language }
            ?: detail.images?.logos?.firstOrNull()
        return logo?.filePath?.let { "$PROFILE_BASE$it" }
    }

    fun getKeywords(detail: TmdbDetail): List<TmdbKeyword> {
        return detail.keywords?.keywords ?: detail.keywords?.results ?: emptyList()
    }
