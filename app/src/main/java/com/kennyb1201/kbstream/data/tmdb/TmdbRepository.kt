...
class TmdbRepository {
    ...
    // Simple in-memory cache for fetchEnrichedMetaCached, keyed by "type:imdbId".
    // Callers like the Continue Watching row can hit this once per show/movie per
    // TTL window instead of re-querying TMDB on every Home refresh.
    private val detailCache = mutableMapOf<String, Pair<Long, TmdbDetail?>>()
    private val detailCacheTtlMs = 12L * 60L * 60L * 1000L // 12 hours
    ...
    // Cached wrapper around fetchEnrichedMeta. Use this for anything called
    // repeatedly for the same title (e.g. Home's Continue Watching row on every
    // resume) rather than the raw call, to avoid hammering TMDB.
    ...
    companion object {
        ...
        // Reasonable poster size for a 150dp-wide TV card without over-fetching.
