...
class IptvRepository(
    ...
    fun observeLineupWithGuide(
        ...
        val guideChannels = playlist.channels
        val channelIds = guideChannels.mapNotNull { it.tvgId?.trim()?.takeIf(String::isNotBlank) }.distinct()

        return dao.observeProgramsForChannelsInWindow(
            channelIds = channelIds,
            windowStart = windowStart,
            windowEnd = windowEnd,
            limit = limit
        ).map { rows ->
            mapChannels(playlist, rows, windowStart)
        }
    }
    ...
    private fun mapChannels(
        ...
        return playlist.channels.map { channel ->
            ...
            val upcoming = matchedPrograms.filter { it.endUtcMillis > nowUtcMillis }.take(12).map {
                XmltvProgram(
