...
class IptvRepository(
    ...
    suspend fun loadLineup(
        ...
        val guide = epgUrl?.let { url ->
            runCatching {
                val guideText = fetchGuideText(url).removePrefix("﻿")
                xmltvParser.parse(
                    guideText,
                    sourceUrl = url
                )
    ...
    private suspend fun fetchGuideText(url: String): String = withContext(Dispatchers.IO) {
        ...
        client.newCall(request).execute().use { response ->
            ...
            Log.d(
                TAG,
                "EPG fetch decoded: url=$url rawBytes=${rawBytes.size} decodedChars=${text.length} gzip=$isGzip preview=${text.take(120).replace("\n", " ")}"
            )
