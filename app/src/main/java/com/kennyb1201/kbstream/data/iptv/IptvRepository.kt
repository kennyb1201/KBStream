...
class IptvRepository(
    ...
    suspend fun importGuide(epgUrl: String) = withContext(Dispatchers.IO) {
        ...
        try {
            val result = runCatching {
                // Clear once before the streamed import. XmltvImporter inserts bounded batches.
    ...
    fun observeLineupWithGuide(
        ...
        Log.d(TAG, "LINEUP QUERY candidates=${channelIds.size} epgUrl=$normalizedGuideUrl")
        ...
        Log.d(TAG, "LINEUP QUERY rows=${rows.size} epgUrl=$normalizedGuideUrl")
        emit(mapChannels(channelKeysByPlaylistChannel, rows, System.currentTimeMillis()))
    ...
    private suspend fun loadProgramsChunked(
        ...
        if (channelIds.isEmpty() || totalLimit <= 0) return emptyList()

        val results = ArrayList<EpgProgramRow>(minOf(totalLimit, INITIAL_PROGRAM_CAPACITY))
    ...
    private fun addCandidateKeys(keys: MutableSet<String>, value: String) {
    ...
    private fun EpgProgramRow.toXmltvProgram(): XmltvProgram = XmltvProgram(
        channelId = channelId,
        title = title,
    ...
    private fun normalizeId(value: String): String =
        value.trim().lowercase(Locale.US).replace(WHITESPACE, "")
    ...
    private fun simplifyName(value: String): String? = normalizeName(value)
        .ifBlank { null }
