...
class SimklRepository(
    ...
    suspend fun getCompletedShowImdbIds(
        ...
        while (true) {
            ...
            Log.e(
                "SIMKL_REPO",
                "getCompletedShowImdbIds page=$page currentPage=$currentPage pageCount=$pageCount pageResults=${body.shows.size} pageIds=${pageIds.size} totalIds=${ids.size}"
            )

            if (body.shows.isEmpty()) {
                break
            }

            if (page >= 20) {
                Log.e("SIMKL_REPO", "getCompletedShowImdbIds stopping at safety page limit")
                break
            }
    ...
    return pairs
        ...
        suspend fun findInWatching(): SimklWatchingShowDetailedItem? {
            ...
            while (true) {
                ...
                Log.e(
                    "SIMKL_REPO",
                    "watching page=$page currentPage=$currentPage pageCount=$pageCount pageResults=${body.shows.size} no match yet for imdb=$imdbId"
                )
                ...
                if (page >= 20) {
                    Log.e("SIMKL_REPO", "findInWatching stopping at safety page limit for imdb=$imdbId")
                    return null
                }
        ...
        suspend fun findInCompleted(): SimklWatchingShowDetailedItem? {
            ...
            while (true) {
                ...
                Log.e(
                    "SIMKL_REPO",
                    "completed page=$page currentPage=$currentPage pageCount=$pageCount pageResults=${body.shows.size} no match yet for imdb=$imdbId"
                )
                ...
                if (page >= 20) {
                    Log.e("SIMKL_REPO", "findInCompleted stopping at safety page limit for imdb=$imdbId")
                    return null
                }
