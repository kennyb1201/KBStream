...
class SimklRepository(
    ...
    suspend fun getContinueWatching(
        ...
        fun isTrulyCompleted(simklId: String): Boolean {
            ...
            val watched = watchingEntry.watchedEpisodesCount
            val total = watchingEntry.totalEpisodesCount
            val notAired = watchingEntry.notAiredEpisodesCount ?: 0
            // total_episodes_count can include episodes/seasons Simkl already knows
            // about but that haven't released yet (e.g. an announced next season).
            // Subtracting not_aired_episodes_count gives the count of episodes that
            // have actually aired -- the real bar for "have you seen everything
            // that's out." nextToWatch being non-blank isn't reliable on its own,
            // since Simkl can point it at an episode that hasn't aired yet.
            val airedTotal = total?.let { it - notAired }
            val caughtUpOnAired = watched != null && airedTotal != null && airedTotal > 0 && watched >= airedTotal

            if (caughtUpOnAired) return true

            // Fall back to the old signal only when we don't have enough data to
            // compute airedTotal.
        ...
        val watchingMapped = watchingShows
            ...
            .filter { item ->
                ...
                // Only count episodes that have actually aired as "owed" to the
                // viewer -- total_episodes_count can include an announced but
                // unreleased next season, and nextToWatch can point at it even
                // though it isn't out yet, so neither is safe to trust alone.
                val airedTotal = total?.let { it - notAired }
                val caughtUpOnAired = watched != null && airedTotal != null && airedTotal > 0 && watched >= airedTotal
                ...
                if (caughtUpOnAired) {
                    ...
                    // No episode-count data to work with -- fall back to Simkl's
                    // own status/nextToWatch signal.
