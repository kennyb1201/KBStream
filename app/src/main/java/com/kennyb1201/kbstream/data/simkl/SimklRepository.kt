...
class SimklRepository(
    ...
    private fun isShowFullyWatched(item: SimklWatchingShowItem): Boolean {
        ...
        // Only evaluate aired counts if airedTotal is confirmed positive to avoid stale metadata blocks
            ...
            .filter { item ->
                ...
                if (status == "dropped" || status == "completed" || status == "ended" || status == "canceled") {
                    ...
                    if (isShowFullyWatched(item)) {
                        ...
                        // Fallback allowance: if fields are sparse/null, still permit inclusion if not explicitly completed
                        true
                    }
                }
            }
    ...
    private fun parseNextTarget(value: String?): Pair<Int, Int?>? {
        ...
        // Use .find() with a flexible pattern to handle formatting variations safely
    ...
    private fun fallbackWatchingTarget(
        ...
        val watchedCount = item.watchedEpisodesCount ?: 0
        if (watchedCount > 0) {
            // If watched count exists but nextToWatch was blank, attempt parsing lastWatched field as a safety fallback
            return parseNextTarget(item.lastWatched)
        }

        val status = item.status?.trim()?.lowercase().orEmpty()
        if (status == "dropped" || status == "completed" || status == "ended" || status == "canceled") return null
