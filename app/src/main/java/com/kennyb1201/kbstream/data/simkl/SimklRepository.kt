...
class SimklRepository(
    ...
    suspend fun getWatchedEpisodesForShowByImdb(
        ...
        val response = api.getWatchingShowsDetailed(
            authorization = bearer(accessToken),
            dateFrom = null,
            extended = "full",
        ...
        val show = response.shows.firstOrNull { item ->
            item.show?.ids?.imdb == imdbId
        }

        Log.e("SIMKL_REPO", "detailed watching lookup imdb=$imdbId found=${show != null}")
        Log.e("SIMKL_REPO", "detailed watching seasons for imdb=$imdbId = ${show?.seasons}")
