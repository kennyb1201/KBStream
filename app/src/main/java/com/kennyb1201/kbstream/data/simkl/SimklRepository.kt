...
class SimklRepository(
    ...
    suspend fun getContinueWatching(
        ...
        val playbackMapped = playbackItems.mapNotNull { item ->
            ...
            when {
                movie != null -> {
                    val simklId = movie.ids?.simkl?.toString() ?: item.id?.toString() ?: return@mapNotNull null
                ...
                show != null -> {
                    val simklId = show.ids?.simkl?.toString() ?: item.id?.toString() ?: return@mapNotNull null
                    SimklContinueWatchingItem(
                        id = "show-$simklId",
                        ...
                        lastWatchedAt = item.pausedAt,
                        progress = item.progress,
                        upNextText = buildString {
                            append("Up next")
                            if (episode?.season != null || episode?.episode != null) {
                                append(": ")
                                episode.season?.let { season -> append("S$season") }
                                episode.episode?.let { ep -> append("E$ep") }
                            }
                            episode?.title
                                ?.takeIf { it.isNotBlank() }
                                ?.let { title ->
        ...
        val watchingMapped = watchingShows.mapNotNull { item ->
    ...
    private fun buildWatchingUpNextText(
        ...
        nextToWatch?.takeIf { it.isNotBlank() }?.let {
            parts += "Next $it"
        }
