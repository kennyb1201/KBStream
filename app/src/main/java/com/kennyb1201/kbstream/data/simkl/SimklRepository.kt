...
class SimklRepository(
    ...
    suspend fun getContinueWatching(
        ...
        fun isTrulyCompleted(simklId: String): Boolean {
            ...
            // Simkl's status often stays "watching" for an ongoing show even once
            // you're caught up on every episode it's aired so far -- it only flips
            // to "completed" once the show itself has ended. Being fully caught up
            // (watched >= total, no next episode queued) means the same thing for
            // our purposes, so treat that as completed too.
            val caughtUp = watched != null && total != null && total > 0 && watched >= total

            return (status == "completed" || caughtUp)
        }
        ...
        val watchingMapped = watchingShows
            ...
            .filter { item ->
                ...
                // "completed" status or being fully caught up on every aired episode
                // both mean the same thing for our purposes — only resurface if Simkl
                // says there's an actual next episode/season waiting.
                status != "dropped" && (hasNext || (status != "completed" && !caughtUp))
            }
            .mapNotNull { item ->
    ...
    private fun buildWatchingUpNextText(
        ...
        if (next.isNotBlank()) {
            ...
            return when {
                normalized.matches(Regex("""s\d+e\d+""")) -> "Next episode • $next"
                normalized.matches(Regex("""s\d+""")) -> "New season • $next"
