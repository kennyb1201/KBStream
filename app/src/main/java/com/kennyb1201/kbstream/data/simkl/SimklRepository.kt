...
class SimklRepository(
    ...
    suspend fun getContinueWatching(
        ...
        return getWatchingShows(
            accessToken = accessToken,
            dateFrom = dateFrom,
            extended = extended
        ).mapNotNull { item ->
            val show = item.show ?: return@mapNotNull null
            val next = item.nextToWatchInfo
                ...
                upNextText = next?.let {
                    buildString {
                        ...
                        if (it.season != null || it.episode != null) {
                            append(": ")
                            it.season?.let { season -> append("S$season") }
                            it.episode?.let { episode -> append("E$episode") }
                        }
                        it.title?.takeIf { title -> title.isNotBlank() }?.let { title ->
                            append(" • ")
                            append(title)
                        }
                    }
