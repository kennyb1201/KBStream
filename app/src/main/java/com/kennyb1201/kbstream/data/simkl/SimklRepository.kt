...
class SimklRepository(
    ...
    suspend fun getContinueWatching(
        ...
        Log.e("SIMKL_SAMPLE", "getContinueWatching started")

        val playbackItems = getPlaybackItems(accessToken)
        val watchingShows = getWatchingShows(accessToken).shows

        Log.e("SIMKL_SAMPLE", "watchingShows count = ${watchingShows.size}")

        val sampleShow = watchingShows.firstOrNull { !it.nextToWatch.isNullOrBlank() }
            ?: watchingShows.firstOrNull()

        if (sampleShow != null) {
            val json = moshi.adapter(SimklWatchingShowItem::class.java)
                .indent("  ")
                .toJson(sampleShow)

            logLongError("SIMKL_SAMPLE", json)
        } else {
            Log.e("SIMKL_SAMPLE", "No watching show sample found")
        }
    ...
    private fun logLongError(tag: String, text: String) {
        val chunkSize = 3000
        var start = 0
