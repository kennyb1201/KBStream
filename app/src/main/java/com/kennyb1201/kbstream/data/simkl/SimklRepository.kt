...
class SimklRepository(
        ...
        val watchingMapped = watchingShows
            ...
            .mapNotNull { item ->
                ...
                SimklContinueWatchingItem(
                    ...
                    progress = null, // Suppress progress bar on watching cards unless active playback exists
    ...
    private fun buildWatchingUpNextText(
        ...
        if (next.isNotBlank()) {
            ...
            return when {
                Regex("""s\d+e\d+""", RegexOption.IGNORE_CASE).matches(normalized) -> "Up next: $next"
                Regex("""s\d+""", RegexOption.IGNORE_CASE).matches(normalized) -> "Up next: $next"
