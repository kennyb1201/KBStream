...
fun ActorScreen(
    ...
    val castCredits by viewModel.castCredits.collectAsState()
    val crewCredits by viewModel.crewCredits.collectAsState()
    ...
    when {
        ...
        person == null -> Box(
            ...
            val featuredCast = remember(castCredits) {
                castCredits
            ...
            val featuredCrew = remember(crewCredits) {
                crewCredits
                    .filter {
                        (it.mediaType == "movie" || it.mediaType == "tv") &&
                            (!it.job.isNullOrBlank() || !it.department.isNullOrBlank())
                    }
                    .distinctBy { "${it.mediaType}:${it.id}:${it.job}:${it.department}" }
                    .take(20)
            }
            ...
            CompositionLocalProvider(LocalBringIntoViewSpec provides LocalTvBringIntoViewSpec) {
                Box(modifier = Modifier.fillMaxSize()) {
                    ...
                    LazyColumn(
                            ...
                            item(key = "cast_row") {
                                LazyRow(
                                    ...
                                    items(
                                        ...
                                        PersonCreditPosterCard(
                                            credit = credit,
                                            subtitle = credit.character,
                                            onClick = {
                                                val navType = if (credit.mediaType == "tv") "series" else "movie"
                                                viewModel.onCreditClick(credit, onNavigateDetail, navType)
                                            }
                                        )
                                    }
                                }
                        ...
                        if (featuredCrew.isNotEmpty()) {
                            ...
                            item(key = "crew_row") {
                                LazyRow(
                                    ...
                                    items(
                                        items = featuredCrew,
                                        key = { "${it.mediaType}:${it.id}:${it.job}:${it.department}" }
                                    ) { credit ->
                                        PersonCreditPosterCard(
                                            credit = credit,
                                            subtitle = credit.job ?: credit.department,
                                            onClick = {
                                                val navType = if (credit.mediaType == "tv") "series" else "movie"
                                                viewModel.onCreditClick(credit, onNavigateDetail, navType)
                                            }
                        ...
                        if (featuredCast.isEmpty() && featuredCrew.isEmpty()) {
                            item(key = "empty") {
                                Text(
...
@Composable
private fun PersonCreditPosterCard(
