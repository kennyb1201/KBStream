...
fun DetailScreen(
    ...
    LaunchedEffect(type, id, seasons, resumeInfo?.season) {
        if (type == "series" && selectedSeason == null && seasons.isNotEmpty()) {
            selectedSeason = resumeInfo?.season?.takeIf { it in seasons } ?: seasons.first()
        }
    }
    ...
    when {
        ...
        meta != null -> {
            ...
            if (type == "movie") {
                ...
                val targetEpisode = targetEpisodeModel?.episodeNumber
                    ?: if (hasResume) resumeEpisode!! else fallbackEpisode
