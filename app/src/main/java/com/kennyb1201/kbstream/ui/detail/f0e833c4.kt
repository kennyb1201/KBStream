...
fun DetailScreen(
    ...
    LaunchedEffect(tmdbDetail, resumeInfo, type) {
        if (selectedSeason == null && seasons.isNotEmpty()) {
            selectedSeason = if (type == "series") {
                resumeInfo?.season?.takeIf { it in seasons } ?: seasons.first()
            } else {
                seasons.first()
            }
            shouldFocusResumeEpisode = true
        }
    }
                            ...
                            if (type == "series" && seasons.isNotEmpty()) {
                                ...
                                item(key = "episodes_row") {
                                    LazyRow(
                                        ...
                                        items(items = episodes, key = { it.streamId }) { ep: ResolvedEpisode ->
                                            val episodeKey = remember(id, selectedSeason, ep.episodeNumber) {
                                                selectedSeason?.let { season -> "$id:$season:${ep.episodeNumber}" }
                                            }

                                            val isEpisodeWatched = remember(episodeKey, watchedEpisodeKeys) {
