                                ...
                                LazyRow(
                                    ...
                                    modifier = Modifier
                                        ...
                                        .onPreviewKeyEvent { keyEvent: KeyEvent ->
                                            if (type == "movie" && keyEvent.type == KeyEventType.KeyDown && keyEvent.key == Key.DirectionDown) {
...
private fun formatSeriesYearRange(detail: TmdbDetail?): String? {
    if (detail == null) return null
    val startYear = extractYear(detail.firstAirDate)
    val endYear = extractYear(detail.lastAirDate)
    val status = detail.status?.trim().orEmpty()
    if (startYear == null) return null
    return when {
        status.equals("Returning Series", ignoreCase = true) ||
            status.equals("In Production", ignoreCase = true) -> "$startYear-"
...
private fun seriesStatusTag(detail: TmdbDetail?): String? {
    val status = detail?.status?.trim().orEmpty()
    return when {
        status.equals("Returning Series", ignoreCase = true) || status.equals("In Production", ignoreCase = true) -> "ONGOING"
        status.equals("Canceled", ignoreCase = true) || status.equals("Cancelled", ignoreCase = true) -> "CANCELLED"
        status.equals("Ended", ignoreCase = true) -> "ENDED"
        else -> null
    }
}
