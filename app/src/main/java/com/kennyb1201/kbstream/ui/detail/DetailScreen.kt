...
fun DetailScreen(
    ...
    val episodeFocusRequesters = remember { mutableMapOf<Pair<Int, Int>, FocusRequester>() }
    ...
    val effectiveSeason = remember(type, selectedSeason, seasons, initialTarget?.season, resumeInfo?.season, vmLoadedSeason, isLoading) {
        if (type != "series") null
        else selectedSeason
            ?: initialTarget?.season?.takeIf { it in seasons }
            ?: resumeInfo?.season?.takeIf { it in seasons }
            // The ViewModel's own auto-selected season (resume-aware, and
            // also aware of Simkl/local watch history that this screen has
            // no visibility into) - prefer it once it's settled.
            ?: vmLoadedSeason?.takeIf { it in seasons }
            // Only fall back to the first season once the initial load has
            // settled - falling back while isLoading is still true risks
            // locking onto season 1 before resumeInfo/loadedSeason have had
            // a chance to report the real season (both are separate async
            // sources that can resolve slightly after `seasons` does).
            ?: seasons.firstOrNull().takeIf { !isLoading }
    }

    // Whether effectiveSeason resolved from an explicit source (user pick,
    // deep link, resume, or the ViewModel's own settled auto-selection)
    // rather than the naive last-resort fallback above. Only these should
    // actively drive a loadEpisodesForSeason call - the naive fallback case
    // means the ViewModel's own autoLoadRelevantSeason call is presumably
    // still in flight, and calling loadEpisodesForSeason again here with a
    // dumber guess can race it and clobber the correct result once it lands.
    ...
    LaunchedEffect(type, id, initialTarget?.season, initialTarget?.episode) {
        ...
        // Stale requesters from a previously-viewed title otherwise linger
        // in these maps for the lifetime of this screen instance.
    ...
    LaunchedEffect(type, id, effectiveSeason, hasExplicitSeasonSource) {
        if (type == "series" && effectiveSeason != null && hasExplicitSeasonSource) {
    ...
    val resolvedTargetEpisode = remember(
        ...
        if (type != "series") {
            ...
            // The ViewModel already picks the right episode for whichever
            // season it most recently loaded - the resume episode if this
            // season has one in progress, otherwise the first unwatched
            // episode. Trust it directly instead of re-deriving the same
            // thing locally (which is what raced resumeInfo's async load).
            // Guard against staleness: only trust it if it's actually part
            // of the currently-loaded episodes list.
            vmTargetEpisode?.takeIf { ep -> episodes.any { it.streamId == ep.streamId } }
                ?: episodes.firstOrNull()
        }
    }

    val targetEpisodeNumber = remember(type, resolvedTargetEpisode) {
        if (type != "series") null else resolvedTargetEpisode?.episodeNumber
    }
    ...
    // Jumps focus straight to the currently targeted episode (resume point,
    // deep-link target, or first unwatched episode) instead of letting the
    // default D-pad focus search land on whatever focusable happens to be
    // nearest below - which is often the wrong season chip or nothing at
    // all if the episode row hasn't scrolled into place yet. Falls back to
    // the current season's chip if the episode isn't focusable yet (e.g.
    // still loading). Returns true if a focus target was found and
    // requested, so the caller knows whether to consume the key event.
    fun focusSeasonOrEpisode(): Boolean {
        val season = effectiveSeason
        val episodeNum = targetEpisodeNumber
        val episodeRequester = if (season != null && episodeNum != null) {
        ...
        val seasonRequester = season?.let { seasonFocusRequesters[it] }
    ...
    LaunchedEffect(effectiveSeason, episodesLoading, episodes, episodeTransitionState.edge) {
        ...
        delay(90)
        runCatching { episodeFocusRequesters[season to targetEpisode.episodeNumber]?.requestFocus() }
    ...
    when {
        ...
        meta != null -> {
            ...
            val keywords = remember(tmdbDetail) { tmdbDetail?.keywords?.list().orEmpty() }
            ...
            if (type == "movie") {
                ...
                val targetSeason = effectiveSeason
                    ?: initialTarget?.season?.takeIf { it in seasons }
                    ?: seasons.firstOrNull()
                    ?: 1
                val targetEpisode = resolvedTargetEpisode?.episodeNumber
                    ...
                    ?: resumeEpisode
                    ?: 1
                val isResumingHere = hasSeriesResume && targetSeason == resumeSeason && targetEpisode == resumeEpisode
                val isDeepLinkedHere = initialTarget?.season == effectiveSeason && initialTarget?.episode != null && !isResumingHere

                val targetStreamId = resolvedTargetEpisode?.streamId
                    ?: (resumeStreamId?.takeIf { isResumingHere })
                    ?: initialTarget?.streamId
                        ?.takeIf { initialTarget.season == targetSeason && initialTarget.episode == targetEpisode }
                    ?: "$id:$targetSeason:$targetEpisode"

                playLabel = when {
                    // Deep-linked to a specific episode that isn't the
                    // resume point - the ViewModel's label doesn't know
                    // about this navigation intent, so build it locally.
                    isDeepLinkedHere -> "PLAY S${targetSeason} E${targetEpisode}"
                    // Otherwise trust the ViewModel's resume/next-up aware
                    // label - it's already correctly computed for whichever
                    // season it most recently loaded. resolvedTargetEpisode
                    // being non-null means that label is for this season.
                    resolvedTargetEpisode != null -> vmPlayButtonText
                    else -> "PLAY"
                }
                ...
                playTarget = remember(
                    ...
                    StreamsTarget(
                        ...
                        resumePositionMs = if (isResumingHere) resumeInfo?.positionMs ?: 0L else 0L
                    )
                }
            }
                ...
                Column(modifier = Modifier.fillMaxSize()) {
                    ...
                    Column(modifier = Modifier.padding(start = 24.dp, end = 24.dp, bottom = 8.dp)) {
                        val metaLine = remember(m, tmdbDetail, type) {
                            val yearInfo = if (type == "series") {
                                formatSeriesYearRange(tmdbDetail?.firstAirDate, tmdbDetail?.lastEpisodeToAir?.airDate, tmdbDetail?.status) ?: tmdbDetail?.releaseYear() ?: m.releaseInfo
                            } else {
                            ...
                            listOfNotNull(
                                ...
                                yearInfo,
                                if (type == "series") seriesStatusTag(tmdbDetail?.status) else null,
                                m.runtime,
                            ...
                            LazyRow(
                                ...
                                modifier = Modifier
                                    ...
                                    .onPreviewKeyEvent { keyEvent: KeyEvent ->
                                        // Only take over here when there's no keywords row
                                        // below to hand off to - keywords row (if present)
                                        // does its own redirect further down.
                                        if (
                                            type == "series" &&
                                            keywords.isEmpty() &&
                    ...
                    LazyColumn(
                        ...
                        val detailFacts = buildList {
                            tmdbDetail?.bestReleaseDate()?.let { formatDisplayDate(it) }
                                ?.let { add(DetailFactItem("Release Date", it)) }
                            if (type == "movie") {
                                tmdbDetail?.budget?.takeIf { it > 0L }?.let { add(DetailFactItem("Budget", formatUsd(it))) }
                                tmdbDetail?.revenue?.takeIf { it > 0L }?.let { add(DetailFactItem("Revenue", formatUsd(it))) }
                            }
                            m.country?.takeIf { it.isNotBlank() }?.let { add(DetailFactItem("Country", it)) }
                            m.language?.takeIf { it.isNotBlank() }?.let { add(DetailFactItem("Language", it)) }
                            m.awards?.takeIf { it.isNotBlank() }?.let { add(DetailFactItem("Awards", it)) }
                        }
                        ...
                        if (detailFacts.isNotEmpty()) {
                            item(key = "detailfacts") {
                                LazyRow(
                                    ...
                                    items(detailFacts, key = { it.label }) { fact ->
                                        DetailFactCard(
                                            fact = fact,
                                            modifier = Modifier.padding(end = 10.dp)
                                        )
                                    ...
                                    modifier = Modifier
                                        ...
                                        .onPreviewKeyEvent { keyEvent: KeyEvent ->
                                            if (keyEvent.type != KeyEventType.KeyDown || keyEvent.key != Key.DirectionDown) {
                                                ...
                                                movieDetailsFocusRequester.requestFocus()
                                                true
                                            } else if (type == "series") {
                                                focusSeasonOrEpisode()
                                            } else {
                                        ...
                                        CompositionLocalProvider(LocalBringIntoViewSpec provides LocalTvBringIntoViewSpec) {
                                            LazyRow(
                                                ...
                                                items(items = episodes, key = { it.streamId }) { ep ->
                                                    ...
                                                    effectiveSeason?.let { season ->
                                                        episodeFocusRequesters[season to ep.episodeNumber] = focusRequester
                                                    }
                        ...
                        if (peopleItems.isNotEmpty()) {
                            item(key = "peopleheader") {
                                Text(
                                    ...
                                    modifier = Modifier
                                        ...
                                        .focusRequester(movieDetailsFocusRequester)
                                        .focusable()
                                )
                            }
                            item(key = "peoplerow") {
                            ...
                            item(key = "castfallback") {
                                Text("Cast ${m.cast.orEmpty().joinToString(", ")}", modifier = Modifier.padding(start = 24.dp, top = 12.dp, end = 24.dp))
                            }
                        } else {
...
/**
 * Shared 124x180 poster tile used by both the collection row and the
 * recommendations row - they only ever differed in how the poster path and
 * content description were pulled off their respective model types.
 */
@Composable
private fun PosterGridCard(posterPath: String?, contentDescription: String, isWatched: Boolean, onClick: () -> Unit) {
    PosterCard(
        posterUrl = remember(posterPath) { posterPath?.let { TmdbRepository.POSTER_BASE + it } },
...
/**
 * Parses an ISO-8601 date string (e.g. TMDB's "yyyy-MM-dd") and formats it
 * as "MM/dd/yyyy". Falls back to the raw input if parsing fails so a
 * malformed or already-formatted string never gets swallowed.
 */
...
private fun formatEpisodeAirDate(raw: String): String = formatDisplayDate(raw) ?: raw
...
private fun extractYear(value: String?): String? =
    value?.takeIf { it.isNotBlank() }?.take(4)?.takeIf { it.length == 4 && it.all(Char::isDigit) }
...
private fun formatSeriesYearRange(
    ...
    val startYear = extractYear(firstAirDate)
    val endYear = extractYear(lastAirDate)
    val normalizedStatus = status?.trim().orEmpty()
    if (startYear == null) return null
    return when {
        normalizedStatus.equals("Returning Series", ignoreCase = true) ||
            normalizedStatus.equals("In Production", ignoreCase = true) -> "$startYear-"
        !endYear.isNullOrBlank() && endYear != startYear -> "$startYear-$endYear"
        !endYear.isNullOrBlank() -> endYear
        else -> "$startYear-"
    }
}
...
private fun seriesStatusTag(status: String?): String? {
    ...
    return when {
        normalizedStatus.equals("Returning Series", ignoreCase = true) ||
            normalizedStatus.equals("In Production", ignoreCase = true) -> "ONGOING"
