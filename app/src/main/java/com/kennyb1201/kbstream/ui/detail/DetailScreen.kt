            ...
            CompositionLocalProvider(LocalBringIntoViewSpec provides LocalTvBringIntoViewSpec) {
                Column(modifier = Modifier.fillMaxSize()) {
                    ...
                    LazyColumn(
                        ...
                        val recs = (tmdbDetail?.recommendations?.results.orEmpty() + tmdbDetail?.similar?.results.orEmpty())
                            .distinctBy { it.id }
                        if (recs.isNotEmpty()) {
                            ...
                            item(key = "recs_row") {
                                LazyRow(
                                    ...
                                    items(items = recs.take(20), key = { it.id }) { rec: TmdbRecommendationItem ->
                                        RecCard(
                                            rec = rec,
                                            onClick = {
                                                scope.launch {
                        ...
                        if (reviews.isNotEmpty()) {
                            ...
                            items(items = reviews.take(5), key = { "review_${it.id}" }) { review: TmdbReview ->
