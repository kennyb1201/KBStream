...
fun DetailScreen(
    ...
    val targetEpisodeNumber = if (type == "series") {
        resumeInfo?.takeIf { it.season == selectedSeason }?.episode
    } else {
        null
    }

    val targetEpisodeIndex = remember(episodes, targetEpisodeNumber) {
        if (targetEpisodeNumber == null) {
            -1
        } else {
            episodes.indexOfFirst { it.episodeNumber == targetEpisodeNumber }
        }
    }
    ...
    LaunchedEffect(type, seasons, resumeInfo?.season) {
        if (type == "series" && seasons.isNotEmpty()) {
            val preferredSeason = resumeInfo?.season?.takeIf { it in seasons } ?: seasons.first()
            if (selectedSeason != preferredSeason) {
                selectedSeason = preferredSeason
            }
        }
    }

    LaunchedEffect(type, selectedSeason) {
        if (type == "series") {
    ...
    LaunchedEffect(selectedSeason, episodesLoading, targetEpisodeIndex) {
        if (
            type == "series" &&
            !episodesLoading &&
    ...
    when {
        ...
        meta != null -> {
            ...
            if (type == "movie") {
                ...
                val fallbackSeason = selectedSeason ?: seasons.firstOrNull() ?: 1
                val fallbackEpisode = episodes.firstOrNull()?.episodeNumber ?: 1
                val resumeEpisodeMatch = episodes.firstOrNull { ep ->
                    resumeStreamId != null && ep.streamId == resumeStreamId
                } ?: episodes.firstOrNull { ep ->
                    resumeEpisode != null && ep.episodeNumber == resumeEpisode
                }

                val targetSeason = if (hasResume) {
                    resumeSeason!!
                } else {
                    selectedSeason ?: fallbackSeason
                }
                val targetEpisodeModel = if (hasResume) {
                    resumeEpisodeMatch
                } else {
                    episodes.firstOrNull()
                }
                val targetEpisode = targetEpisodeModel?.episodeNumber ?: if (hasResume) resumeEpisode!! else fallbackEpisode
                val targetStreamId = targetEpisodeModel?.streamId
                    ?: resumeStreamId
                        ...
                        LazyColumn(
                            ...
                            item(key = "info_block") {
                                Column(
                                    ...
                                    tmdbDetail?.bestReleaseDate()?.let {
                                        Text(
                                            "Release Date: " + it,
                                            color = KBTextLo,
                                ...
                                item(key = "episodes_row") {
                                    LazyRow(
                                        ...
                                        items(
                                            ...
                                            val shouldFocusThisCard = ep.episodeNumber == targetEpisodeNumber

                                            LaunchedEffect(shouldFocusThisCard, episodesLoading) {
                                                if (shouldFocusThisCard && !episodesLoading) {
                                            ...
                                            EpisodeCard(
                                                ...
                                                onClick = {
                                                    ...
                                                    val target = StreamsTarget(
                                                        ...
                                                        title = displayName + " · S" + (selectedSeason ?: 1) + "E" + ep.episodeNumber + epSuffix,
                                                        displayName = displayName,
