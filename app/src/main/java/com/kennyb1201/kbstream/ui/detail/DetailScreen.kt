...
fun DetailScreen(
    ...
    val clearLogoUrl = remember(tmdbDetail) { tmdbDetail?.bestLogoPath()?.let { TmdbRepository.BACKDROP_BASE + it } }
    ...
    when {
        ...
        meta != null -> {
            ...
            val backdropUrl = remember(m) { m.background ?: m.poster ?: initialPoster }
                                        ...
                                        item(key = "episodesrow") {
                                            LazyRow(
                                                ...
                                                items(items = episodes, key = { it.streamId }) { ep ->
                                                    ...
                                                    LaunchedEffect(shouldFocusThisCard, episodesLoading, effectiveSeason, episodes.size, userManuallyChangedSeason) {
                                                        if (shouldFocusThisCard && !episodesLoading && episodes.isNotEmpty() && !userManuallyChangedSeason) {
