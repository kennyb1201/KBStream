...
data class DetailReviewItem(
    ...
    val rawReview: TmdbReview? = null
)

private fun TmdbReview.toDetailReviewItem(): DetailReviewItem {
    val ratingLabel = authorDetails?.rating
        ?.takeIf { it > 0.0 }
        ?.let { rating ->
            if (rating % 1.0 == 0.0) "${rating.toInt()}/10" else String.format(Locale.US, "%.1f/10", rating)
        }

    return DetailReviewItem(
        id = id,
        author = author.ifBlank { "Anonymous" },
        source = "TMDb",
        content = content,
        ratingLabel = ratingLabel,
        publishedLabel = createdAt?.take(10)?.takeIf { it.isNotBlank() },
        rawReview = this
    )
}
...
fun DetailScreen(
    ...
    var selectedBiography by remember { mutableStateOf<TmdbCastMember?>(null) }
    var selectedEpisode by remember { mutableStateOf<ResolvedEpisode?>(null) }
    ...
    val selectedPersonDetail by viewModel.selectedPersonDetail.collectAsState()
    val selectedPersonLoading by viewModel.selectedPersonLoading.collectAsState()
    val selectedPersonError by viewModel.selectedPersonError.collectAsState()


    val reviewItems = remember(tmdbDetail?.reviews?.results) {
        tmdbDetail?.reviews?.results.orEmpty().map { it.toDetailReviewItem() }
    }
    ...
    LaunchedEffect(effectiveSeason, episodesLoading, resolvedTargetEpisode?.streamId, episodes) {
        if (type == "series" && !episodesLoading && episodes.isNotEmpty()) {
            val current = selectedEpisode
            if (current == null || episodes.none { it.streamId == current.streamId }) {
                selectedEpisode = resolvedTargetEpisode ?: episodes.first()
            }
        }
    }
    ...
    when {
        ...
        meta != null -> {
            ...
            if (type == "movie") {
                ...
                val playSeason = effectiveSeason
                    ?: selectedSeason
                    ?: initialTarget?.season
                    ?: resumeSeason
                    ?: seasons.firstOrNull()
                    ?: 1
                val playEpisode = selectedEpisode?.episodeNumber
                    ?: resolvedTargetEpisode?.episodeNumber
                    ?: if (hasSeriesResume && playSeason == resumeSeason) resumeEpisode else null
                    ?: initialTarget?.episode
                    ?: episodes.firstOrNull()?.episodeNumber
                    ?: 1
                val playStreamId = selectedEpisode?.streamId
                    ?: resolvedTargetEpisode?.streamId
                    ?: if (hasSeriesResume && playSeason == resumeSeason) resumeStreamId else null
                    ?: initialTarget?.streamId?.takeIf { initialTarget?.season == playSeason && initialTarget?.episode == playEpisode }
                    ?: "$id:$playSeason:$playEpisode"
                val shouldResumeThisEpisode = hasSeriesResume && playSeason == resumeSeason && playEpisode == resumeEpisode

                val playLabelPrefix = if (shouldResumeThisEpisode) "RESUME" else "PLAY"
                playLabel = "$playLabelPrefix S${playSeason} E${playEpisode}"
                playTarget = remember(
                    resumeInfo,
                    id,
                    displayName,
                    ...
                    StreamsTarget(
                        contentType = "series",
                        streamId = playStreamId ?: "$id:$playSeason:$playEpisode",
                        title = "$displayName S$playSeason E$playEpisode",
                        displayName = displayName,
                        season = playSeason,
                        episode = playEpisode,
                        resumePositionMs = if (shouldResumeThisEpisode) resumeInfo?.positionMs ?: 0L else 0L
                    )
                }
            }
                ...
                Box(modifier = Modifier.fillMaxSize()) {
                                                    ...
                                                    EpisodeCard(
                                                        ...
                                                        modifier = Modifier
                                                            ...
                                                            .onFocusChanged { focusState ->
                                                                if (focusState.isFocused) selectedEpisode = ep
                                                            }
                                                            .onKeyEvent { keyEvent ->
                                ...
                                item(key = "peoplerow") {
                                    LazyRow(
                                        ...
                                        items(items = peopleItems, key = { person ->
                                            ...
                                            when (person) {
                                                is PeopleRowItem.Person -> CastCard(
                                                    member = person.member,
                                                    onClick = { onNavigateActor(person.member.id) },
                                                    onShowBiography = {
                                                        selectedBiography = person.member
                                                        viewModel.loadPerson(person.member.id)
                                                    }
                                                )
                                ...
                                item(key = "reviewsrow") {
                                    LazyRow(
                                        ...
                                        items(reviews.take(10), key = { it.id }) { review ->
                                            ReviewCard(review = review, onClick = { review.rawReview?.let { selectedReview = it } })
                                        }
                                    }
                                }
                            }
                    ...
                    selectedBiography?.let { member ->
                        BiographyOverlay(
                            member = member,
                            personDetail = selectedPersonDetail,
                            isLoading = selectedPersonLoading,
                            error = selectedPersonError,
                            onDismiss = { selectedBiography = null }
                        )
                    }
                }
            }
...
private fun ReviewCard(
    ...
    Card(
        ...
        Column(modifier = Modifier.padding(16.dp)) {
                ...
                Row(verticalAlignment = Alignment.CenterVertically) {
                    Box(
                        modifier = Modifier
                            ...
                            .background(KBAccent.copy(alpha = 0.18f))
                            .padding(horizontal = 10.dp, vertical = 4.dp)
                    ) {
                        Text(
                            text = review.source.uppercase(),
                            color = KBAccent,
                    ...
                    review.publishedLabel?.let { published ->
                        Text(
                            text = published,
                            color = KBTextLo,
                            style = MaterialTheme.typography.bodySmall,
            ...
            Text(
                text = review.content.ifBlank { "No review text available." },
                color = KBTextLo,
                maxLines = 5,
                overflow = TextOverflow.Ellipsis,
...
@Composable
private fun BiographyOverlay(
    member: TmdbCastMember,
    personDetail: com.kennyb1201.kbstream.data.tmdb.TmdbPersonDetail?,
    isLoading: Boolean,
    error: String?,
    onDismiss: () -> Unit
) {
    var expanded by remember(member.id) { mutableStateOf(false) }
    val biography = personDetail?.biography?.takeIf { it.isNotBlank() }
    val shouldShowSeeMore = (biography?.length ?: 0) > 420
        ...
        Card(
            ...
            Column(modifier = Modifier.padding(24.dp)) {
                ...
                member.character?.takeIf { it.isNotBlank() }?.let {
                    Text(text = it, color = KBTextLo, modifier = Modifier.padding(top = 4.dp))
                }
                Spacer(modifier = Modifier.height(16.dp))
                when {
                    isLoading -> Text("Loading biography...", color = KBTextLo)
                    error != null -> Text(error, color = KBTextLo)
                    biography != null -> {
                        Text(
                            text = biography,
                            color = KBTextLo,
                    ...
                    else -> Text("No biography available.", color = KBTextLo)
                }
