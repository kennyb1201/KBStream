...
fun DetailScreen(
    ...
    when {
        ...
        meta != null -> {
            ...
            val logoUrl = remember(tmdbDetail) {
                tmdbDetail?.let { viewModel.tmdbRepository.getBestLogo(it) }
            }
            val certification = remember(tmdbDetail, type) {
                tmdbDetail?.let { viewModel.tmdbRepository.getCertification(it) }
            }
            val keywords = remember(tmdbDetail) {
                tmdbDetail?.let { viewModel.tmdbRepository.getKeywords(it) } ?: emptyList()
            }
            val writers = remember(tmdbDetail) {
                tmdbDetail?.let { viewModel.tmdbRepository.getWriters(it) } ?: emptyList()
            }
            val composer = remember(tmdbDetail) {
                tmdbDetail?.let { viewModel.tmdbRepository.getComposer(it) }
            }
            val altTitles = remember(tmdbDetail) {
                tmdbDetail?.let { viewModel.tmdbRepository.getAlternativeTitles(it) } ?: emptyList()
            }
            ...
            CompositionLocalProvider(LocalBringIntoViewSpec provides LocalTvBringIntoViewSpec) {
                Column(modifier = Modifier.fillMaxSize()) {
                    ...
                    Column(modifier = Modifier.padding(start = 24.dp, end = 24.dp, bottom = 8.dp)) {
                        val metaLine = remember(m, certification) {
                            listOfNotNull(
                                certification,
                                m.releaseInfo,
                                m.runtime,
                    ...
                    LazyColumn(
                        ...
                        if (writers.isNotEmpty()) {
                            item(key = "writers") {
                                Text(
                                    "Writers: ${writers.distinctBy { it.id }.joinToString(", ") { it.name }}",
                                    color = KBTextLo,
                        ...
                        if (altTitles.isNotEmpty()) {
                            item(key = "alt_titles") {
                                Text(
                                    "Also known as: ${altTitles.take(3).joinToString(", ")}",
