            ...
            CompositionLocalProvider(LocalBringIntoViewSpec provides LocalTvBringIntoViewSpec) {
                Column(modifier = Modifier.fillMaxSize()) {
                    ...
                    LazyColumn(
                        ...
                        val tmdbWriters = tmdbDetail?.credits.writers()
                        if (tmdbDirector != null || tmdbWriters.isNotEmpty()) {
                            tmdbDirector?.let {
                                item(key = "director") {
                                    Text(
                                        "Director: " + it.name,
                            ...
                            if (tmdbWriters.isNotEmpty()) {
                                item(key = "writers") {
                                    Text(
                                        "Writers: " + tmdbWriters.distinctBy { it.id }.joinToString(", ") { it.name },
                                        color = KBTextLo,
