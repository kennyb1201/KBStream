                        ...
                        LazyColumn(
                            ...
                            item(key = "infoblock") {
                                Column(modifier = Modifier.padding(start = 24.dp, end = 24.dp, bottom = 20.dp)) {
                                    m.description?.takeIf { it.isNotBlank() }?.let { synopsis ->
                                        Text(
                                            synopsis,
                                    ...
                                    val facts = remember(m, tmdbDetail, type) {
                                        buildList {
                                            tmdbDetail?.bestReleaseDate()?.let { add("Release Date" to it) }
                                            if (type == "movie") {
                                                tmdbDetail?.budget?.takeIf { it > 0L }?.let { add("Budget" to formatUsd(it)) }
                                                tmdbDetail?.revenue?.takeIf { it > 0L }?.let { add("Revenue" to formatUsd(it)) }
                                            }
                                            m.country?.let { add("Country" to it) }
                                            m.language?.let { add("Language" to it) }
                                            m.awards?.let { add("Awards" to it) }
                                        }
                                    }
                                    ...
                                    if (facts.isNotEmpty()) {
                                        Column(
                                            ...
                                            facts.chunked(2).forEachIndexed { index, rowFacts ->
