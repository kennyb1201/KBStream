                    ...
                    LazyColumn(
                        ...
                        if (companies.isNotEmpty() || networks.isNotEmpty()) {
                            item(key = "studio_header") {
                                Text(
                                    text = if (type == "series") "NETWORKS" else "PRODUCTION",
                                    style = MaterialTheme.typography.titleMedium,
                                    color = KBTextLo,
                        ...
                        if (collectionParts.isNotEmpty()) {
                            item(key = "collection_header") {
                                Text(
                                    text = collection?.name?.uppercase() ?: "COLLECTION",
                                    style = MaterialTheme.typography.titleMedium,
                                    color = KBTextLo,
                                    modifier = Modifier.padding(start = 24.dp, top = 20.dp, bottom = 10.dp)
                                )
                            ...
                            item(key = "collection_row") {
                                LazyRow(
                                    ...
                                    items(items = collectionParts, key = { it.id }) { part: TmdbCollectionPart ->
                                        val imdbId = resolvedPosterIds[
                                            viewModel.posterLookupKey(part.id, "movie")
                                        ]
                                        val watched = imdbId != null &&
                                            viewModel.watchedKey(imdbId, "movie") in watchedKeys
                                        ...
                                        CollectionCard(
                                            ...
                                            onClick = {
                                                scope.launch {
                                                    val navImdbId = viewModel.resolveImdbId(part.id, "movie")
                                                    if (navImdbId != null) {
                                                        onNavigateDetail("movie", navImdbId)
                                                    }
                                                }
                                            }
                                        )
                            ...
                            item(key = "recs_row") {
                                LazyRow(
                                    ...
                                    items(items = recs.take(15), key = { it.id }) { rec: TmdbRecommendationItem ->
                                        ...
                                        val imdbId = resolvedPosterIds[
                                            viewModel.posterLookupKey(rec.id, mediaType)
                                        ]
                                        val watched = imdbId != null &&
                                            viewModel.watchedKey(imdbId, mediaType) in watchedKeys
                                        ...
                                        RecCard(
                                            ...
                                            onClick = {
                                                scope.launch {
                                                    val navImdbId = viewModel.resolveImdbId(rec.id, mediaType)
                                                    if (navImdbId != null) {
                                                        onNavigateDetail(mediaType, navImdbId)
                                                    }
                                                }
                                            }
                                        )
