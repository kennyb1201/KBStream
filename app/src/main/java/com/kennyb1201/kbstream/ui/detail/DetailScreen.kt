                    ...
                    Column(modifier = Modifier.padding(start = 24.dp, end = 24.dp, bottom = 8.dp)) {
                        ...
                        if (tmdbGenres.isNotEmpty()) {
                            LazyRow(
                                ...
                                items(items = tmdbGenres, key = { it.id }) { genre ->
                                    KBCard(
                                        onClick = { onNavigateTag(genre.id, genre.name, false) },
                        ...
                        if (keywords.isNotEmpty()) {
                            item(key = "keywords_row") {
                                LazyRow(
                                    ...
                                    items(items = keywords, key = { it.id }) { kw ->
                                        KBCard(
                                            onClick = { onNavigateTag(kw.id, kw.name, true) },
                                            modifier = Modifier.padding(end = 8.dp)
                                        ) {
                                            Text(
                                                kw.name,
